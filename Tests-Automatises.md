...
## Nouvelle catégorie de mobilier urbain — recyclage (2026-08-24)

3 nouveaux types ajoutés en fin de liste (Recyclage verre/électronique/autre), icônes violettes (`#7b1fa2` — d'abord turquoise `#00695c`, ajusté le même jour suite au retour utilisateur, cf. section suivante) avec symbole de recyclage classique (3 flèches, construites une seule fois en `<defs>`/`<use>` puis dupliquées à 0°/120°/240°) et un petit pictogramme distinctif par type. Le point d'attention explicite de la demande (aucun impact sur les mobiliers déjà enregistrés) tient par construction : les 5 types existants ne sont ni renommés ni retirés d'aucune structure (liste déroulante, `FICHIERS_ICONE_MOBILIER`, `CATEGORIES_FILTRE`, `COUCHES_MOBILIER`) — seulement complétées en fin de liste.

**Vérification manuelle des icônes** (le rendu visuel n'est pas observable côté Claude, cf. limitation déjà connue) : XML validé sans erreur (`DOMParser`), puis rendu réel analysé par échantillonnage exhaustif des pixels d'un canvas 32×40 (taille réellement utilisée par `L.icon`, pas la taille par défaut d'une `<img>` sans `width`/`height` explicites, qui avait d'abord faussé un premier essai) : couleur dominante `#00695c` (fond), blanc en second (flèches + picto), le reste étant de l'anti-aliasing normal aux bords — cohérent avec le design voulu.

7 nouveaux tests : présence/visibilité par défaut des 3 nouvelles catégories dans le filtre ; raccourcis clavier `6`/`7`/`8` (extension de `1`-`5` à `1`-`8`) ; export/import fidèle des 3 nouveaux types (round-trip dédié) ; **test de non-régression explicite** — un mobilier d'un type existant avant cet ajout (`Corbeille`) survit à l'export/import et conserve son icône inchangée, démontrant directement le point d'attention de la demande. Couverture étendue par ailleurs sans test dédié supplémentaire : le test générique d'existence des fichiers SVG et celui des couleurs de fond couvrent automatiquement les nouveaux fichiers (listes dynamiques, `Object.values(FICHIERS_ICONE_MOBILIER)`).

**Suite complète : 114/114 tests OK** (+ 1 ignoré), rejouée deux fois de suite. Vérification manuelle complémentaire sur l'app réelle : les 8 options apparaissent dans le bon ordre dans la liste déroulante, création d'un point "Recyclage électronique" confirmée bout en bout (donnée enregistrée, marqueur affiché, bonne icône).

## Ajustement de couleur — icônes recyclage (2026-08-24, suite immédiate)

Retour utilisateur : le turquoise (`#00695c`) restait trop proche du vert foncé du banc (`#1b5e20`) et du vert clair du commerce occupé (`#4caf50`). Remplacé par un **violet** (`#7b1fa2`), sans lien de teinte avec aucune couleur déjà utilisée dans la palette (verts, bleus, orange, marron, rouge, gris). Seule la couleur change — structure des icônes, tests, code non affectés autrement que par la valeur attendue dans le test de couleurs. Service worker passé en `v6`.

Vérification manuelle par échantillonnage exhaustif des pixels (même méthode que précédemment) : couleur dominante confirmée `rgb(123,31,162)` = `#7b1fa2`, comparée directement à `banc.svg` (`rgb(27,94,32)`) et `commerce-occupe.svg` (`rgb(76,175,80)`) — nettement distincte des deux. **Suite complète : 115/115 tests OK**, rejouée deux fois de suite.

## Audit externe (RapportAuditGuideCommun.md, 2026-08-24)

Rapport basé sur `GuideCommunRelectureCode.md`, appliqué au 23/08 (lecture de code, sans exécution) — 4 points relevés :

1. **Collision d'`uid` entre onglets du même appareil** (corrigé) — `genererUid()` (`app/js/device.js`) lisait/incrémentait/réécrivait `compteurLocal` dans `localStorage` en trois opérations non atomiques ; deux onglets créant un objet à quelques millisecondes d'intervalle pouvaient produire le même `uid`. Corrigé en sérialisant ce bloc via `navigator.locks` (Web Locks API) quand disponible, avec repli sans verrou identique au comportement précédent sinon. `genererUid()` devient async — mis à jour dans ses 2 appelants (`mobilier.js`, `commerce.js`).
   - Limite assumée du test : le vrai scénario (deux onglets = deux contextes JS séparés) n'est pas simulable dans ce harnais (tout tourne dans un seul fil JS, où deux appels "concurrents" ne s'entrelacent jamais même sans verrou). 4 tests portent donc sur ce qui est réellement vérifiable : garde-fou code manquant, fonctionnement normal, compteurs strictement croissants sur plusieurs appels successifs, et **`navigator.locks.request` bien appelé** (vérifié par substitution temporaire de la fonction) — confirme le branchement correct plutôt que l'absence de la race elle-même.
2. **Confidentialité des tuiles OSM** (documenté) — limite déjà inhérente à l'usage direct d'OpenStreetMap, pas un bug ; ajoutée à `Specifications.md` §2 comme limite connue et acceptée.
3. **3 trous de couverture déjà signalés** (déjà résolus) — le rapport datant du 23/08, il ne tenait pas compte des corrections apportées le 24/08 dans la même journée (service worker isolé en fonctions testables, échec IndexedDB sur la suppression directe, bouton Annuler). Aucune action supplémentaire nécessaire.
...
## Effacer les données locales (2026-09-12)

Cf. Specifications.md §6.5ter, pour le mode opératoire "quartier par quartier" (§2). Nouvelle fonction `effacerDonnees()` (`app/js/gpkg.js`), réutilisant `viderStore()` (déjà utilisée par "Remplacer" à l'import) et `viderMarqueurs()` (déjà utilisée pour retirer les marqueurs affichés). 3° bouton "Effacer" dans le menu Fichier (PC et mobile). 4 nouveaux tests fonctionnels :

- **Refus (confirm annulé)** : rien n'est supprimé, ni en base ni sur la carte.
- **Acceptation** : les 2 bases (`mobilier_urbain`, `commerce`) sont vidées, tous les marqueurs (mobilier et commerce) sont retirés de la carte — **et** `localStorage` (`code_appareil`, `compteur_local`) reste strictement inchangé, vérifié par comparaison avant/après. C'est le point le plus important de cette fonctionnalité : sans cette garantie, la consolidation par fusion successive des quartiers (§2) ne fonctionnerait plus sans collision de `uid`.
- **Message de confirmation explicite** : contient bien un rappel à l'export ("exporté") et la mention "irréversible", conformément à la demande explicite de l'utilisateur plutôt qu'un verrou technique (ex. suivre si un export a eu lieu).
- **Câblage du bouton mobile** (écran Fichier) : vérifié par substitution temporaire de `effacerDonnees`, même principe que pour `exporterDonnees` déjà en place.

**Bug de test trouvé et corrigé en cours de route** (pas un bug de l'app) : les deux premiers tests écrits laissaient `confirmReponse` à `false` sans le restaurer à `true` en fin de test, contrairement à la convention déjà suivie partout ailleurs dans la suite. Conséquence concrète observée : deux tests sans rapport, plus loin dans la suite (mémorisation du dernier type créé), échouaient par timeout (`attendreCondition : délai dépassé`) — la sauvegarde silencieusement bloquée par un `confirm()` de doublon répondant `false` à cause de cette pollution d'état. Diagnostiqué en comparant le comportement à l'exécution isolée des mêmes étapes, puis corrigé en restaurant `confirmReponse = true` à la fin des deux tests concernés, conformément à la convention. Signalé aussi car révélateur pour la suite : `confirmReponse` est un état global partagé entre tous les tests de ce fichier, à réinitialiser systématiquement après l'avoir modifié.

**Suite complète : 137/137 tests OK**, rejouée deux fois de suite. Vérification manuelle complémentaire sur l'app réelle (PC et mobile) : bouton "Effacer" visible aux deux formats, clic réel déclenchant bien une confirmation native (pas de suppression silencieuse sans confirmation), écran Fichier mobile affichant les 3 boutons dans l'ordre attendu. Service worker passé en `v11`.
